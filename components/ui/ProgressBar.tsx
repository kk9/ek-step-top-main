...
const ProgressBar = () => {
  ...
  return (
    <div className="absolute bottom-0 w-screen p-10 pb-[3.5rem] font-Rubik font-bold text-white bg-gradient-to-t from-[#554342] via-[#554342] to-transparent">
      <div className="flex w-full cursor-pointer">
        {introDone &&
          categories.map((category, index) => (
            <div className={"flex flex-col items-center justify-center w-[15%] "} key={index} onClick={() => setCurrent(category.name)}>
              <div className="shadow text-sm lg:text-lg h-4 mb-4">{index + 1}</div>
              <div
                key={index}
              ...
              ></div>
              <div className="shadow text-sm lg:text-lg h-4">{category.name}</div>
            </div>
          ))}
      </div>
    </div>
